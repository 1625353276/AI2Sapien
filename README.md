...
- [x] 整门课程知识分析：30,000 字符文档批次、最多 3 路并发、来源页码与原文引文校验
...
课程分析只把 PDF 的嵌入文本按文档批次发送给当前选中的模型提供者，不上传原始 PDF，也不执行 OCR。每个模型返回的概念必须包含页码和原文短引，本地校验通过后才会进入知识地图。关闭应用会停止当前进程；已完成批次已经保存在本机，下次点击“继续分析”会复用断点，不会从头开始。
